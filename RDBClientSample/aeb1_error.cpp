                           ...
                           bool stopNow = false) 
{
    ...
    if (stopNow) {
        ...
        drv->brakePedal = brakeTgt;   // full brake
        fprintf(stderr, "[CTRL] Full brake applied (brakePedal=%.2f)\n", brakeTgt);
    } else {
...
int main(int argc, char* argv[])
{
    ...
    const float fwd_max_range_m = 140.0f;  // ahead within this range
    const float kp_steer = 0.5f;
            ...
            if (ego.has) {
                ...
                float brakeCmd = 0.0f;
                    ...
                    printf("Front Object (ID=%d, Name=%s)  dist=%.2f m, x_fwd=%.2f, y_left=%.2f\n",
                     ...
                     float rel_vx = ego.vx - bestFront.vx;
                      const float maxDecel = 6.0f;     // m/s², typical emergency decel
                      const float safetyMargin = 2.0f;
                        printf("Front Object (ID=%d, Name=%s)  dist=%.2f m, " "x_fwd=%.2f, y_left=%.2f, rel_vx=%.2f\n", bestFront.id, bestFront.name[0] ? bestFront.name : "N/A",dist_xy, x_local, y_local, rel_vx);
                    ...
                    accelCmd = 0.0f;
                    printf("No vehicle ahead in gate → accelTgt=%.2f\n", accelCmd);
                }
